...
if __name__ == "__main__":
    ...
    for route in routes:
        ...
        # write result to csv per route, to the same csv
        if all_results:
            all_results_df = pd.concat(all_results)
            all_results_df.to_csv(f"data/results-04-15.csv", mode='a+', index=False, header=route_count==1)
            all_results = []
        route_count += 1
